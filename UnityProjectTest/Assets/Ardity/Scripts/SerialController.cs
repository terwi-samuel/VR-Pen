...
public class SerialController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        string[] xy_positions = { "0.00000", "0.00000" };
        int i = 0;
        foreach(string position in xy)
        {
            xy_positions[i] = position;
            i++;
        }
        Debug.Log(xy_positions[0]);
        Debug.Log(xy_positions[1]);

        CultureInfo ci = (CultureInfo)CultureInfo.CurrentCulture.Clone();
        ci.NumberFormat.CurrencyDecimalSeparator = ".";
        float x;
        float y;
        if(float.TryParse(xy_positions[0], NumberStyles.Any, ci, out x))
        {
            Debug.Log("Successfully parsed x");
        }
        else
        {
            Debug.Log("Coult not parse parsed x");
        }
        if (float.TryParse(xy_positions[1], NumberStyles.Any, ci, out y))
        {
            Debug.Log("Successfully parsed y");
        }
        else
        {
            Debug.Log("Coult not parse parsed y");
        }
        transform.position = new Vector3(x*10, y*10, (float)0);
